...
contract VideoShare {
    ...
    struct User {
        ...
        mapping (uint => uint) txts;
        mapping (uint => uint) pics;
    }
    struct Txt {
        string content;
        bool only;
    ...
    mapping (uint => Txt) txts;
    mapping (uint => Picture) pics;
    ...
    function getUserInfo (address _userAdd) view public returns (string memory nickname, string memory profile, 
        string memory avatar, uint videoNums) {
        return (users[_userAdd].nickname, users[_userAdd].profile, users[_userAdd].avatar, users[_userAdd].videoNums);
    }
    function setTxt (uint _txtId, string memory _content, bool _only,uint8 _permission) public {
        if (msg.sender != txts[_txtId].author) return;
        txts[_txtId].content = _content;
        txts[_txtId].permission = _permission;
    }
    function getTxt (uint _txtId) view public returns (string memory content) {
        if (txts[_txtId].permission == 0 || msg.sender == txts[_txtId].author) {
            Txt storage _txt = txts[_txtId];
            
            return (_txt.content);
        }
    }
    
    function setPicture (uint _picId, string memory _pic, bool _only,uint8 _permission) public {
        if (msg.sender != pics[_picId].author) return;
        pics[_picId].pic = _pic;
        pics[_picId].permission = _permission;
    }
    function getPicture (uint _picId) view public returns (string memory pic) {
        if (pics[_picId].permission == 0 || msg.sender == pics[_picId].author) {
            Picture storage _pic = pics[_picId];
            
            return (_pic.pic);
        }
    }

    function publish (string memory _title, string memory _cover, string memory _videoinfo, string memory _info, 
        uint _duration, string memory _filename, string memory _fileinfo, uint _size, uint32 _width, uint32 _height, uint32 _fps) 
    public returns (uint videoId) {
